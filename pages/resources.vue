<template>
  <main>
    <AppHeader title="الموارد والأدوات" :description="description" eyebrow="أدلة ومصادر" />
      ...
      <div class="max-w-site mx-auto px-6 sm:px-8 lg:px-12">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-4xl mx-auto">
          <a
            ...
            target="_blank"
            rel="noopener"
            class="group block bg-cream-deep hover:bg-[#ECECED] border border-black/[0.03] rounded-[20px] p-7 transition-all duration-300"
          >
            <div class="flex items-start justify-between gap-4 mb-4">
              <div
                class="w-11 h-11 rounded-full bg-white flex items-center justify-center flex-shrink-0 shadow-[0_2px_8px_rgba(0,0,0,0.04)]"
              >
            ...
            <p class="text-[14px] text-ink-soft leading-relaxed mb-4">
              {{ resource.description }}
            </p>

            <div class="flex items-center gap-2 text-xs">
              <span
                class="px-3 py-1 rounded-full bg-white text-ink font-medium shadow-[0_1px_4px_rgba(0,0,0,0.02)]"
              >
                {{ resource.tag }}
              </span>
